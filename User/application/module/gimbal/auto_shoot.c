...
// 单例模式，静态全局变量
static auto_shoot_t s_auto_shoot;
...
// 跟踪控制状态，定义为静态变量避免全局污染
static uint8_t s_control_status = 0;
static float s_allow_error_distance = 0.09f;
static float s_fly_time;
static float s_pre_xc;
static float s_pre_yc;
static float s_pre_yaw;
...
void auto_shoot_solve_trajectory(solver_track_t *solver_track, send_packed_t *send_packed) {
    ...
    s_fly_time = distance / horizontal_speed + solver_track->bias_time / 1000.0f;

    // 计算飞行时间后的装甲板位置
    s_pre_xc = xc + solver_track->vxw * s_fly_time;
    s_pre_yc = yc + solver_track->vyw * s_fly_time;
    s_pre_yaw = armor_yaw + solver_track->v_yaw * s_fly_time;
