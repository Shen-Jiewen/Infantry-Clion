...
void MahonyAHRSinit(float ax, float ay, float az, float mx, float my, float mz) {
	...
	const float init_pitch = atan2f(-ax, az);
	const float init_roll = atan2f(ay, az);

	const float sin_roll = sinf(init_roll);
	const float cos_roll = cosf(init_roll);
	const float cos_pitch = cosf(init_pitch);
	const float sin_pitch = sinf(init_pitch);

	if (mx != 0.0f && my != 0.0f && mz != 0.0f) {
		const float magX = mx * cos_pitch + my * sin_pitch * sin_roll + mz * sin_pitch * cos_roll;
		const float magY = my * cos_roll - mz * sin_roll;
		init_yaw = atan2f(-magY, magX);
	} else {
		init_yaw = 0.0f;
	}

	const float cr2 = cosf(init_roll * 0.5f);
	const float cp2 = cosf(init_pitch * 0.5f);
	const float cy2 = cosf(init_yaw * 0.5f);
	const float sr2 = sinf(init_roll * 0.5f);
	const float sp2 = sinf(init_pitch * 0.5f);
	const float sy2 = sinf(init_yaw * 0.5f);
